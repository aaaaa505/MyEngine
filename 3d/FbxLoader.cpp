...
//
//    //スケール、回転、平行移動行列の計算
//    XMMATRIX matScaling, matRotaion, matTranslation;
//    matScaling = XMMatrixScalingFromVector(node.scaling);
//    matRotaion = XMMatrixRotationRollPitchYawFromVector(node.rotation);
//    matTranslation = XMMatrixTranslationFromVector(node.translation);
//
//    //ローカル変形行列の計算
//    node.transform = XMMatrixIdentity();
//    node.transform *= matScaling;//ワールド行列にスケーリングを反映
//    node.transform *= matRotaion;//ワールド行列に回転を反映
...
//        //XMMatrix型に変換する
//        XMMATRIX initiaPoes;
//        ConvertMatrixFromFbx(&initiaPoes, fbxMat);
//
//        //初期姿勢行列の逆行列を得る
//        bone.invInitialPose = XMMatrixInverse(nullptr, initiaPoes);
//    }
//
//    //ボーン番号とスキンウェイトのペア
//    struct WeightSet
...
//        {
//            //頂点データに書き込み
//            vertices[i].boneIndex[weightArrayindex] = weightSet.indedx;
